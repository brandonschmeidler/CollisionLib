...
local function bind(f, vector) return function(...) return f(vector, ...) end end
local default_filter = function() return true end
local function deep_copy(t, dest, aType) t = t or {}; local r = dest or {}; for k,vector in pairs(t) do if aType ~= nil and type(vector) == aType then r[k] = (type(vector) == 'table') and ((_classes[vector] or _instances[vector]) and vector or deep_copy(vector)) or vector elseif aType == nil then r[k] = (type(vector) == 'table') and k~= '__index' and ((_classes[vector] or _instances[vector]) and vector or deep_copy(vector)) or vector end; end return r end
